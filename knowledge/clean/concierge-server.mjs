...
// Knowledge-first (NO hallucinations): lists/recommendations come from knowledge/verified.json
    ...
    const hardRules = (CONCIERGE_FULL_GPT
      ? [
    ...
    // In FULL GPT mode: strip URLs from the text to avoid hallucinated links in the UI.
    const reply = CONCIERGE_FULL_GPT ? stripUrlsFromText(rawReply) : rawReply;
