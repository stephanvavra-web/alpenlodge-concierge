...
// The concierge must ONLY recommend items that exist in our local knowledge JSON files.
//
// IMPORTANT: Alle Knowledge-JSONs im Ordner `knowledge/` werden aktiv geladen (Dateinamen egal).
// Optional: `KNOWLEDGE_FILE` kann auch auf *eine einzelne* JSON zeigen (Legacy/Debug).
// Default: `<repo>/knowledge` (Directory-Mode).
const KNOWLEDGE_FILE = process.env.KNOWLEDGE_FILE || path.join(__dirname, "knowledge"); // file OR directory

let _knowledgeCache = { signature: "", value: null, files: [], skipped: [] };

function _isDir(p) {
  try { return fs.statSync(p).isDirectory(); } catch { return false; }
}
function _safeBaseName(p) {
  try { return path.basename(p); } catch { return String(p || ""); }
}
function _resolveKnowledgePath() {
  return path.isAbsolute(KNOWLEDGE_FILE) ? KNOWLEDGE_FILE : path.join(process.cwd(), KNOWLEDGE_FILE);
}

function listKnowledgeJsonFiles() {
  const p = _resolveKnowledgePath();

  // Directory-mode: load ALL *.json files in that directory
  if (_isDir(p)) {
    const files = fs.readdirSync(p)
      .filter((f) => f.toLowerCase().endsWith(".json"))
      .filter((f) => !f.startsWith("."))
      .filter((f) => !f.startsWith("._")) // macOS Finder junk
      .map((f) => path.join(p, f))
      .sort((a, b) => _safeBaseName(a).localeCompare(_safeBaseName(b), "de"));
    return { mode: "dir", base: p, files };
  }

  // File-mode (legacy)
  return { mode: "file", base: path.dirname(p), files: [p] };
}

function makeKnowledgeSignature(filePaths) {
  const parts = [];
  for (const fp of filePaths) {
    try {
      const st = fs.statSync(fp);
      parts.push(`${_safeBaseName(fp)}:${st.mtimeMs}:${st.size}`);
    } catch {
      parts.push(`${_safeBaseName(fp)}:missing`);
    }
  }
  return parts.join("|");
}
...
function dedupeKeyForItem(it) {
  const url = String(it?.url || it?.website || "").trim();
  const src = String(it?.sourceUrl || it?.source || "").trim();
  const u = url || src;
  if (u.startsWith("http://") || u.startsWith("https://")) return `url:${u}`;

  const name = dedupeText(it?.name || it?.title || "");
  const lat = Number(it?.lat);
  const lon = Number(it?.lon);
  if (name && Number.isFinite(lat) && Number.isFinite(lon)) return `geo:${name}:${lat.toFixed(5)}:${lon.toFixed(5)}`;
  if (name) return `name:${name}`;

  return `json:${JSON.stringify(it).slice(0, 220)}`;
}

function mergeNormalizedKnowledge(parts) {
  const out = { categories: {}, directories: [], alpenlodge: null, meta: {} };

  const dirSeen = new Set();
  const itemSeen = new Map(); // cat -> Set

  const addDir = (d) => {
    if (!d) return;
    const url = String(d.url || d.href || "").trim();
    if (!url || !(url.startsWith("http://") || url.startsWith("https://"))) return;
    if (dirSeen.has(url)) return;
    dirSeen.add(url);
    out.directories.push({
      label: String(d.label || d.name || url),
      url,
      category: d.category || null,
    });
  };

  const mergeAlpenlodge = (a) => {
    ...
    // Merge amenities (best effort, dedupe by name)
    const a0 = out.alpenlodge;
    const amen0 = Array.isArray(a0.amenities) ? a0.amenities : [];
    const amen1 = Array.isArray(a.amenities) ? a.amenities : [];
    if (amen1.length) {
      const seen = new Set(amen0.map((x) => dedupeText(x?.name || x?.title || "")));
      const merged = [...amen0];
      for (const x of amen1) {
        const k = dedupeText(x?.name || x?.title || "");
  ...
  for (const n of (Array.isArray(parts) ? parts : [])) {
    if (!n) continue;

    if (n.meta && typeof n.meta === "object") out.meta = { ...out.meta, ...n.meta };
    mergeAlpenlodge(n.alpenlodge);

    (Array.isArray(n.directories) ? n.directories : []).forEach(addDir);

    for (const [cat, arr] of Object.entries(n.categories || {})) {
      if (!Array.isArray(arr)) continue;
      if (!out.categories[cat]) out.categories[cat] = [];
      const seen = itemSeen.get(cat) || new Set();
      itemSeen.set(cat, seen);
      for (const it of arr) {
        if (!it || typeof it !== "object") continue;
        const key = dedupeKeyForItem(it);
...
function loadKnowledge() {
  try {
    const info = listKnowledgeJsonFiles();
    const signature = `${info.mode}|${info.base}|${makeKnowledgeSignature(info.files)}`;
    if (_knowledgeCache.value && _knowledgeCache.signature === signature) return _knowledgeCache.value;

    const loaded = [];
    const skipped = [];

    for (const fp of info.files) {
      try {
        const raw = fs.readFileSync(fp, "utf8");
        const json = JSON.parse(raw);

        // normalizeKnowledge() returns null if it doesn't look like a knowledge schema
        const norm = normalizeKnowledge(json);
        if (!norm) {
          skipped.push({ file: _safeBaseName(fp), reason: "not_knowledge_schema" });
          continue;
        }

        loaded.push({ file: _safeBaseName(fp), norm });
      } catch (e) {
        skipped.push({ file: _safeBaseName(fp), reason: "parse_error" });
      }
    }

    const merged = mergeNormalizedKnowledge(loaded.map((x) => x.norm));

    // Attach debug info (safe, no secrets)
    merged._debug = {
      mode: info.mode,
      base: info.base,
      files: loaded.map((x) => ({
        file: x.file,
        directories: Array.isArray(x.norm.directories) ? x.norm.directories.length : 0,
        categories: Object.fromEntries(
          Object.entries(x.norm.categories || {}).map(([k, v]) => [k, Array.isArray(v) ? v.length : 0])
        ),
      })),
      skipped,
    };

    _knowledgeCache = { signature, value: merged, files: merged._debug.files, skipped };
    return merged;
  } catch (e) {
    return null;
  }
...
// B) { items: [...], sources: {...}, directories?: [...], alpenlodge?:{...}, center?:{lat,lon}, meta?:{...} }
// C) { highlights: { ... }, official_portals?: [...], meta?:{...} }
...
function normalizeKnowledge(raw) {
  ...
  // ---------------- Format C (concierge_knowledge_de_verified.json): highlights + official portals
  if (raw.highlights && typeof raw.highlights === "object") {
    ...
    const mapHighlightKeyToCat = (k) => {
      const key = String(k || "").toLowerCase();
      if (key === "water") return "lakes_pools_wellness";
      if (key === "wellness_pools") return "lakes_pools_wellness";
      if (key === "ski") return "ski";
      if (key === "rodel") return "rodel";
      if (key === "family") return "family";
      return "activities";
    };

    // Official portals -> directories
    const portals = Array.isArray(raw.official_portals) ? raw.official_portals : [];
    portals.forEach((p) => {
      const url = p?.url || p?.href;
      const name = p?.name || p?.label || "Offizielles Portal";
      if (typeof url === "string" && (url.startsWith("http://") || url.startsWith("https://"))) {
        out.directories.push({ label: String(name), url: url.trim(), category: "all" });
      }
    });

    // Highlights -> categories
    for (const [k, arr] of Object.entries(raw.highlights || {})) {
      if (!Array.isArray(arr)) continue;
      const cat = mapHighlightKeyToCat(k);
      for (const x of arr) {
        if (!x || typeof x !== "object") continue;
        push(cat, {
          name: x.name || x.title || "Tipp",
          summary: x.summary || x.details || x.area || "",
          url: x.url || x.href || null,
          sourceUrl: x.sourceUrl || x.source || null,
          type: String(k || ""),
        });
      }
    }
  ...
  // ---------------- Format A (already normalized): categories + directories
  if (raw.categories && typeof raw.categories === "object") {
    // Avoid accidentally treating config JSON as knowledge:
    // require at least one array-of-objects with a `name`/`title`, OR directories present.
    const cats = raw.categories;
    const hasCategoryItems = Object.values(cats).some((v) =>
      Array.isArray(v) && v.some((it) => it && typeof it === "object" && (it.name || it.title))
    );
    const hasDirs = Array.isArray(raw.directories) && raw.directories.some((d) => d && (d.url || d.href));

    if (hasCategoryItems || hasDirs) {
      return {
  ...
  // ---------------- Format B: items + sources/directories
  const items = Array.isArray(raw.items) ? raw.items : null;
  const sources = raw.sources && typeof raw.sources === "object" ? raw.sources : null;
  const dirs = Array.isArray(raw.directories) ? raw.directories : null;

  if (!items && !sources && !dirs) return null;
  ...
  const mapSourceKeyToCat = (key) => {
    const k = String(key || "").toLowerCase();
    if (k === "ski_areas") return "ski";
    if (k === "doctors_pharmacies") return "medical";
    return k;
  };

  // Directories from `sources` (split multi-URL strings)
  if (sources) {
    for (const [key, v] of Object.entries(sources)) {
      if (!v) continue;
      const label = v.label || key;
      const urlRaw = v.url || v.source;
      const cat = mapSourceKeyToCat(key);

      // One source entry can contain multiple URLs separated by "|"
      const urls = splitHttpUrls(urlRaw);
      if (urls.length) {
        urls.forEach((u) => out.directories.push({ label, url: u, category: cat }));
      } else if (typeof urlRaw === "string" && (urlRaw.startsWith("http://") || urlRaw.startsWith("https://"))) {
        out.directories.push({ label, url: urlRaw.trim(), category: cat });
      }
    }
  }

  // Directories from `directories` array (various schemas)
  if (dirs) {
    for (const d0 of dirs) {
      if (!d0 || typeof d0 !== "object") continue;
      const url = d0.url || d0.href;
      if (typeof url !== "string" || !(url.startsWith("http://") || url.startsWith("https://"))) continue;

      const label = d0.label || d0.name || "Link";
      let cat = d0.category || null;

      const id = String(d0.id || "").toLowerCase();
      const nameL = String(label || "").toLowerCase();
      if (!cat) {
        if (id.includes("event") || nameL.includes("event") || nameL.includes("veranstalt")) cat = "events";
        else if (id.includes("gastro") || nameL.includes("gastro") || nameL.includes("kulinar") || nameL.includes("restaurant")) cat = "restaurants";
        else if (id.includes("tour") || nameL.includes("tour") || nameL.includes("wandern") || nameL.includes("hike")) cat = "hiking";
        else if (id.includes("ski") || nameL.includes("ski")) cat = "ski";
        else cat = "all";
      }

      out.directories.push({ label: String(label), url: url.trim(), category: cat });
    }
  }
  ...
  const mapTypeToCat = (it) => {
    ...
    if (type.includes("rodel") || type.includes("toboggan") || type.includes("sled")) return "rodel";
    ...
    if (type.includes("hiking") || type.includes("hike") || type.includes("trail") || type.includes("tour")) return "hiking";
    ...
    // Fallback: still keep "sight/nature/winter/..." as general activities
    if (["sight","nature","winter","card"].some((k) => type.includes(k))) return "activities";

    return null;
  };

  for (const it0 of (Array.isArray(items) ? items : [])) {
...
function detectCategory(userText) {
  ...
  // Water + wellness are one combined category in the verified knowledge dumps
  if (/(badesee|see\b|baden|schwimmen|strand|badestrand|wasser)/i.test(t)) return "lakes_pools_wellness";
  if (/(hallenbad|therme|wellness|spa|sauna|infrarot|pool)/i.test(t)) return "lakes_pools_wellness";
  ...
  if (/(wandern|wanderweg|hike|hiking|spazier|winterwander|schneeschuh|langlauf|trail|tour)/i.test(t)) return "hiking";
  if (/(verleih|rental|mieten|skiverleih|ski\s*verleih|bike\s*verleih|fahrradverleih)/i.test(t)) return "rental";

  if (/(familie|kinder|kids|spielplatz|kletter|kinderfreund|kind)/i.test(t)) return "family";
  if (/(ausflug|aktivitaet|aktivitaeten|sehenswuerd|museum|festung|glas|manufaktur|was\s+tun)/i.test(t)) return "activities";
...
function buildCategoryReply(cat, kRaw, radiusKm = 35, sessionId = "") {
  ...
  bayern_daytrips: "Bayern – Tagesausflüge",
  activities: "Ausflüge & Aktivitäten",
  family: "Familie & Kids",
  shopping: "Shopping",
  nightlife: "Nightlife",
  alpenlodge: "Alpenlodge (Ausstattung & Hausinfos)",

  // legacy aliases (falls alte Keys in JSONs stehen)
  skigebiete: "Skigebiete",
  lakes: "Seen & Wellness",
  wellness_pools: "Seen & Wellness",
  ski_areas: "Skigebiete",
  doctors_pharmacies: "Ärzte & Apotheken",
};
  ...
  if (shown.length) {
    ...
    const dirs = Array.isArray(k.directories)
      ? k.directories.filter((d) => d && isHttpUrl(d.url) && (d.category === cat || d.category === "all" || !d.category))
      : [];
...
app.get("/api/debug/knowledge", (req, res) => {
  ...
  const dbg = raw && typeof raw === "object" ? (raw._debug || null) : null;
